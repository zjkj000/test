...
export default function CorrectingPaper(props) {
    ...
    useEffect(() => {
        navigation.setOptions({
            title: props.route.params.userCn,
            // headerRight:()=>(<Menu getselectedindex={setSelectedIndex} learnPlanId={props.route.params.learnId}/>)
        });

        props.route.params.CorrectResultList
            ? setCorrectResultList(props.route.params.CorrectResultList)
            : setCorrectResultList([]);
        getData();

        // console.log('props.route.params.correctingstatus',props.route.params.correctingstatus)
        if (
            props.route.params.correctingstatus == "4" ||
            props.route.params.correctingstatus == "5"
        ) {
            Alert.alert("", "该学生已批改！是否直接查看批改结果？", [
                {},
                {
                    text: "否",
                    onPress: () => {
    ...
    function changestatus() {
        const url =
            global.constants.baseUrl + "teacherApp_deleteAccessControl.do";
        const params = {
    ...
    function getData() {
        const url =
            global.constants.baseUrl + "teacherApp_correctingHomeWork.do";
        const params = {
    ...
    function update_StuAnswer(str1, str2) {
        ...
        var newdata = data;
        newdata[selectedIndex].stuAnswer = newdata[
            selectedIndex
        ].stuAnswer.replace(str1, str2);
        setData([]);
        setData(newdata);
    ...
    function lodaingSubmitPager(success) {
        if (success) {
            return (
                <Layout style={styles.tab} level="2">
                    <CorrectSubmitContainer
                        CorrectResultList={CorrectResultList}
    ...
    function loadingButton(ststus) {
        if (ststus) {
                    ...
                    >
                        <Button
                            style={{
                                width: "40%",
                                backgroundColor:
                                    selectedIndex == 0 ? "#A9A9A9" : "#62C3E4",
                            }}
                            onPress={() => {
                                Thepreviousquestion(selectedIndex - 1);
                            }}
                        >
                            上一题
                        </Button>
            ...
            return (
                <View style={{ backgroundColor: "#FFFFFF", height: 80 }}></View>
            );
        }
    }
    ...
    function submit_correctResult() {
        ...
        const url =
            ...
            "teacherApp_saveHomeWorkCorrectResult.do";
        const params = {
            taskId: taskId, //作业id或者导学案id
            type: type, //  paper,表示作业；learnPlan表示导学案
