...
class CreateHomework extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            paperId:
                this.props.paramsData.type == "create"
                    ? ""
                    : this.props.paramsData.paperId, //空试卷id

            addPaperFlag: this.props.paramsData.type == "create" ? true : false, //导航“添加试题”是否被选中
            updatePaperFlag:
                this.props.paramsData.type == "create" ? false : true, //导航“调整顺序”是否被选中
    ...
    UNSAFE_componentWillMount() {
        ...
        _dateStr =
            _dateStr.substring(0, 11) +
            (parseInt(_dateStr.substring(12, 13)) + 8) +
            _dateStr.substring(13, 16);
        this.setState({
            startTime: _dateStr,
        ...
        http.get(url, params)
            .then((resStr) => {
                ...
                if (selectPaperListCopy.length > 0) {
                    ...
                    for (let i = 1; i < selectPaperListCopy.length; i++) {
                        //获取选中题目的baseTypeId个数
                        if (
                            selectPaperListCopy[i].baseTypeId.indexOf(
                                baseTypeIdtemp
                            ) < 0
                        ) {
                            if (
                                baseTypeIdList.indexOf(
                                    selectPaperListCopy[i].baseTypeId
                                ) < 0
                            ) {
                                baseTypeIdList.push(
                                    selectPaperListCopy[i].baseTypeId
    ...
    createPaperObject = () => {
        ...
        for (let i = 0; i < baseTypeIdLists.length; i++) {
            //baseTypeIdLists和selectPaperList中的baseTypeId顺序一致
            ...
            for (j; j < selectPaperList.length; j++) {
                ...
                if (
                    selectPaperList[j].baseTypeId.indexOf(baseTypeIdLists[i]) <
                    0
                ) {
                    bigId++;
                    smallId = 1;
    ...
    setPushPapersParams = () => {
        ...
        var classIdOrGroupId = classSecleted.classId.substring(
            0,
            classSecleted.classId.length - 1
        ); //班级id
        var classOrGroupName = classSecleted.className.substring(
            0,
            classSecleted.className.length - 1
        ); //班级名(接口返回的班级名后面自带一个逗号,)
        ...
        if (assigntoWho == "0") {
            ...
            classIdOrGroupId = classIdOrGroupId.substring(
                0,
                classIdOrGroupId.length - 1
            );
            classOrGroupName = classOrGroupName.substring(
                0,
                classOrGroupName.length - 1
            );
            ...
            for (let i = 0; i < studentSelected.length; i++) {
                console.log(
                    "****个人******stuId**id****",
                    studentSelected[i].id
                );
                console.log(
                    "****个人******stuNames**name****",
                    studentSelected[i].name
                );
    ...
    ifSelected = () => {
        ...
        for (i = 0; i < selectPaperList.length; i++) {
            if (
                selectPaperList[i].questionId ==
                paperList[selectPaperIndex].questionId
            ) {
                return true;
            }
        }
    ...
    sortSelectPaperList = () => {
        ...
        if (selectPaperListCopy.length > 0) {
            var baseTypeIdtemp = selectPaperListCopy[0].baseTypeId;
            baseTypeIdList.push(baseTypeIdtemp);
            for (let i = 0; i < selectPaperListCopy.length; i++) {
                //获取选中题目的baseTypeId个数
                if (
                    selectPaperListCopy[i].baseTypeId.indexOf(baseTypeIdtemp) <
                    0
                ) {
                    if (
                        baseTypeIdList.indexOf(
        ...
        this.setState(
            {
                ...
                baseTypeIdLists:
                    baseTypeIdList.length > 0 ? baseTypeIdList : [],
            },
            () => {
    ...
    deletePaperTitle = () => {
        ...
        if (updatePaperIndex >= 0) {
            ...
            this.setState({
                ...
                updatePaperIndex:
                    updatePaperIndex == selectPaperList.length
                        ? selectPaperList.length - 1
                        : updatePaperIndex,
                selectPaperList: selectPaperListCopy,
                baseTypeIdLists: baseTypeIdList,
            });
                        ...
                        onPress={() => {
                            ...
                            if (
                                ...
                                if (currentBottomPage >= 0) {
                                    this.setState({
                                        selectPaperIndex: currentBottomPage * 5,
                                    });
                                } else {
    ...
    updateSelectPaperIndex = () => {
        ...
        if (
            tempIndex < paperList.length &&
            selectPaperIndex != tempIndex &&
            paperList.length != 0
        ) {
            console.log(
                "===============需要更新selectPaperIndex======================"
            );
    ...
    showPaperTypeImg = () => {
        ...
        for (
            let i = index;
            i < index + 5 && i >= 0 && i < paperList.length;
            i++
        ) {
            let paperTypeImg;
        ...
        console.log(
            "&&&&&&&&&&currentBeginPaperIndex&&currentLastPaperIndex&&&",
            currentBeginPaperIndex,
            ...
            http.get(url, params)
                .then((resStr) => {
                    ...
                    if (paperListOne == null) {
                        ...
                        paperLength =
                            paperListOne != "" ? paperListOne.length : 0;
                    }
                    console.log(
                        "***pageNo**currentFecthLength***",
                        paperNum,
                        paperLength,
                        Date.parse(new Date())
                    );
                    ...
                    if (paperLength < 5) {
                        ...
                        console.log(
                            "==================所有试题都加载完了===========================",
                            this.state.paperList.length + paperLength,
                            Date.parse(new Date())
                        );
                    }
                    ...
                    this.setState(
                        {
                            paperList: isRefresh
                                ? paperListOne
                                : this.state.paperList.concat(paperListOne),
                            showFoot: foot,
                        },
                        () => {
    ...
    moveUpPaper = () => {
        ...
        if (updatePaperIndex == 0) {
            ...
            const baseTypeId2 =
                selectPaperList[updatePaperIndex - 1].baseTypeId;
            if (baseTypeId1 != baseTypeId2) {
                ...
                tempPaperList[updatePaperIndex] =
                    tempPaperList[updatePaperIndex - 1];
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={{ padding: 10 }}>
                    <RenderHtml
                        contentWidth={screenWidth}
                        source={{
                            html: selectPaperList[updatePaperIndex].tiMian,
                        }}
                    ></RenderHtml>
                </View>
                ...
                <View style={{ padding: 10 }}>
                    <RenderHtml
                        contentWidth={screenWidth}
                        source={{
                            html: selectPaperList[updatePaperIndex].answer,
                        }}
                    ></RenderHtml>
                </View>
                <View style={{ height: 1, backgroundColor: "#999999" }} />
                ...
                <View style={{ padding: 10 }}>
                    <RenderHtml
                        contentWidth={screenWidth}
                        source={{
                            html: selectPaperList[updatePaperIndex].analysis,
                        }}
                    ></RenderHtml>
                </View>
            </ScrollView>
            ...
            paperItems.push(
                ...
                >
                    <Image
                        source={paperTypeImg}
                        style={
                            this.state.updatePaperIndex == paper_i
                                ? styles.checked
                                : styles.little_image
                        }
                    />
    ...
    setStartTime = (time) => {
        this.setState({
            startTime: time,
            endTime: this.nextDay(time.substring(0, 10)),
        });
    };
        ...
        return (
            <View style={{ ...styles.bodyView, height: "82%" }}>
                ...
                <ScrollView style={{ height: "100%" }}>
                    ...
                    >
                        <Text style={{ fontSize: 15, marginRight: 40 }}>
                            开始时间:
                        </Text>
                        ...
                        >
                            ...
                            >
                                <Image
                                    ...
                                    source={
                                        this.state.classNameVisibility
                                            ? require("../../../../assets/image3/top.png")
                                            : require("../../../../assets/image3/bot.png")
                                    }
                                ></Image>
                            </TouchableOpacity>
                        </View>
                    ...
                    >
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                this.updateAssign("0");
                                this.setState({ SelectKeTangStatus: false });
                            }}
                        >
                            <View
                                style={{
