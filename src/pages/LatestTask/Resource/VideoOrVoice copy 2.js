...
export default class VideoOrVoice extends Component {
                ...
                this.state.isPlaying ? null :
                  <TouchableWithoutFeedback onPress={() => { this.onPressPlayButton() }}>
                    <Image
                      ...
                      source={require('../../../assets/LatestTaskImages/mp3.png')}
                    />
              ...
              <View style={[styles.control, {width: this.state.videoWidth}]}>
                <TouchableOpacity activeOpacity={0.3} onPress={() => { this.onControlPlayPress() }}>
                  <Image
                    ...
                    source={this.state.isPlaying ? require('../../../assets/LatestTaskImages/mp3.png') : require('../../../assets/LatestTaskImages/mp3.png')}
                  />
                </TouchableOpacity>
                ...
                <Slider
                  ...
                  thumbImage={require('../../../assets/LatestTaskImages/mp3.png')}
                ...
                <TouchableOpacity activeOpacity={0.3} onPress={() => { this.onControlShrinkPress() }}>
                  <Image
                    ...
                    source={this.state.isFullScreen ? require('../../../assets/LatestTaskImages/mp3.png') : require('../../../assets/LatestTaskImages/mp3.png')}
                  />
                </TouchableOpacity>
              </View> : null
          }
  ...
  _onLoaded = (data) => {
    console.log('视频加载完成');
    this.setState({
      duration: data.duration,  //duration视频总时长
    });
  };
  ...
  _onProgressChanged = (data) => {
    ...
    if (this.state.isPlaying) {
      this.setState({
        currentTime: data.currentTime, //视频当前播放时间
      })
    }
  };
  ...
  _onPlayEnd = () => {
    ...
    this.setState({
      currentTime: 0,
      isPlaying: false,
      playFromBeginning: true  //从头播放设置为true
    });
  };
  ...
  /// 控制播放器工具栏的显示和隐藏
  hideControl() {
    if (this.state.showVideoControl) {  // 是否显示视频控制组件
      this.setState({
  ...
  onPressPlayButton() {
    ...
    this.setState({
      isPlaying: isPlay,
      showVideoCover: false  //已经点过开始播放就不再显示封面
    });
    if (this.state.playFromBeginning) {  //是否从头播放
      this.videoPlayer.seek(0);
      this.setState({
        playFromBeginning: false,  //只有视频播放结束后才会设置为true
      })
    }
  }
  ...
  onControlShrinkPress() {
    if (this.state.isFullScreen) { //当前是横屏
