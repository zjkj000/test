...
<<<<<<< HEAD
    function updateInform() {
        const url =
            "http://www.cn901.com/ShopGoods/ajax/livePlay_editZbLive.do";
        const params = {
            roomId: roomId,
        };
...
<<<<<<< HEAD
    saveLiveingLession() {
        const url =
            "http://www.cn901.com/ShopGoods/ajax/ livePlay_saveZbLive.do";
        const params = {
            userId: global.constants.userName,
            title: "",
            subjectId: "",
    ...
    fetchTea_Info() {
        const url =
            "http://www.cn901.com/ShopGoods/ajax/livePlay_getMyKtListRN.do";
        const params = { userId: global.constants.userName }; // 教师用户名
        http.get(url, params).then((resStr) => {
            let resJson = JSON.parse(resStr);
            this.setState({
                mList: resJson.mList,
                subjectList: resJson.subjectList,
            });
        });
    }
    ...
    renderAvatar = () => {
        return (
            <TouchableOpacity
                onPress={() => this.setState({ moduleVisible: true })}
            >
                <Text
                    style={{ fontSize: 15, color: "#87CEFA", marginLeft: 10 }}
                >
                    {this.state.xzzName == ""
                        ? "------请选择------"
                        : this.state.xzzName}
...
<<<<<<< HEAD
    render() {
        ...
        if (this.state.subjectList.length > 1) {
            for (
                let item_xknum = 0;
                item_xknum < this.state.subjectList.length;
                item_xknum++
            ) {
                Xueke_RadioItem.push(
                    <Radio
                        ...
                        checked={
                            ...
                            this.state.subjectList[item_xknum].subjectId
                                ? true
                                : false
                        }
                        onChange={() => {
                            if (
                                ...
                                this.setState({
                                    subjectId:
                                        this.state.subjectList[item_xknum]
                                            .subjectId,
                                    ketangId: "",
                                });
                            }
                    ...
                    >
                        {this.state.subjectList[item_xknum].subjectName}
                    </Radio>
                );
            }
        } else {
            if (this.state.subjectList.length > 0) {
                ...
                Xueke_RadioItem.push(
                    <Text>{this.state.subjectList[0].subjectName}</Text>
                );
            }
        }
        ...
        for (
            let item_Ktnum = 0;
            item_Ktnum < this.state.ketangList.length;
            item_Ktnum++
        ) {
            Ketang_RadioItem.push(
                <CheckBox
                    ...
                    style={{ height: 30 }}
                    checked={
                        this.state.ketangId.indexOf(
                            this.state.ketangList[item_Ktnum].key
                        ) > -1
                            ? true
                            : false
                    }
                    onChange={() => {
                        ...
                        if (
                            ...
                            var repstr =
                                "," + this.state.ketangList[item_Ktnum].key;
                            this.setState({
                                ketangId: newketangId.replace(repstr, ""),
                            });
                        } else {
                ...
                >
                    {this.state.ketangList[item_Ktnum].value}
                </CheckBox>
            );
        }
...
<<<<<<< HEAD
                        this.setState({
                            moduleVisible: false,
                            xzzName: this.state.mList[item_num].value,
                            xzzId: this.state.mList[item_num].key,
                        });
=======
